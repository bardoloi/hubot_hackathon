...
#   hn[i] - refer to the ith item on hn (starts at 1)
#
# Author:
#   skimbrel
    ...
    msg.http(hnFeedUrl).get() (err, res, body) ->
      if res.statusCode is not 200
        ...
        try
          ...
          msg.send "#{item.getTitle()} (#{item.getComments()?.html})\n#{item.getPermalink()}" for item in items
        catch e
          console.log(e)
          msg.send "Something's gone awry"
